...
import shutil
import sys
sys.path.append("C:\\Users\\take\\PycharmProjects\\ICT_competition_mysql\\data_prep\\DB_MGR")
...
def bizinfo_get_attm_string_from_file(df):
    ...
    attm_list = []
    for idx, value in enumerate(tqdm(df["공고상제URL"])):
        target_url = df["공고상제URL"].iloc[idx]
        ...
        if len(download_button) != 0:
            ...
            try:
                if file_extension == "hwp":
                    ...
                    with open(download_folder_path + "tmp.txt", "w", encoding="utf-8") as f:
